...
_start:
    ...
    cmp eax, 3  
    je parse_program_arguments ;If we have the correct number of arguments, jump.
...
parse_program_arguments:
    ...
    call cStrtoul	;call function to convert port string to work
    add esp, 4	;remove top 4 bytes from the extended stack pointer
    mov [ipPort], eax	;move the contents of eax into the address of the ipPort variable
    ; Check for errors
    cmp eax, 0	;compare eax to zero
    ...
    invalid_program_arguments:	;fall into invalid_program_arguments if eax is not greater than zero
    push msgInvalidArguments	;push the invalid arguments message on the stack
    push msgInvalidArgumentsLen	;push the message length
    call cWriteString	;call the function to write the string
    add esp, 8	;remove top 8 bytes from stack pointer
    call cExit	;call function to exit the program
...
network_connect:
    ...
    ; Low byte second
    mov ax, [ipPort]	;move ipPort variable value into az
    ...
    mov esi, ipOctets	;move the ipOctets variable value into esi
    ; movsb * 4 = movsd
    ...
    rep stosb	;repeat string operation unto remaining bytes are zeroed out

    ; Setup the array that will hold the arguments for connect 
    ; we are passing through the socketcall() system call.
    mov edi, connectArgs	;move connectArgs variable value into edu
    ; sockfd
    mov eax, [sockfd]	;move the address of sockfd into eax
    ...
    mov ecx, connectArgs	;move connectArgs variable contents into ecx
    int 0x80	;call kernel
    
    ; Check if connect() returned a success
    cmp eax, 0	;compare eax to zero
    jge network_setup_file_descriptors	;if eax is greater than zero jump to network_setup_file_descriptors
...
network_setup_file_descriptors:
    ...
    mov edi, masterReadFdArray	;mov masterReadFdArray into edi
...
network_read_write_loop:
    ...
    jg check_read_file_descriptors	; if eax greater than zero jump to check_read_file_descriptors
    ...
    check_socket_file_descriptor:
        ...
        jne check_socket_file_descriptor_done ;if not equal jump to check_socket_file_descriptor_done
        ; Otherwise, it is set, and we need to read the data into a 
